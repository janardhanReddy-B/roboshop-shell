echo -i -e "\e[33mINSTALLING GOLANG\e[0m"
...
echo -i -e "\e[33mUNZIP DISPATCH CONTENT\e[0m"
unzip /tmp/dispatch.zip &>>/tmp/roboshop.log
cd /app &>>/tmp/roboshop.log

echo -i -e "\e[33m INITIALIZE AND AND BUILD DISPATCH\e[0m"
go mod init dispatch &>>/tmp/roboshop.log
go get &>>/tmp/roboshop.log
go build &>>/tmp/roboshop.log

echo -i -e "\e[33mCOPYING GOLANG SERVICE\e[0m"
cp /home/centos/roboshop-shell/dispatch.service /etc/systemd/system/dispatch.service &>>/tmp/roboshop.log

echo -i -e "\e[33mSTARTING GOLANG\e[0m"
